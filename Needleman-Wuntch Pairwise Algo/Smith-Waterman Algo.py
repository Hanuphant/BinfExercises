...
for column in range(2, len(seq2)+2):
    ...
    matrix[1][column] = matrix[1][column-1]
...
def scoreofcell(mat, row, column,mat2):
    score = max(mat[row-1][column-1]+matcher(mat[row][0],mat[0][column]), mat[row-1][column]+gap,mat[row][column-1]+gap,0)
...
for row in range(2, len(matrix)):
    for column in range(2, len(matrix[0])):
        ...
        if maxpossible <= matrix[row][column]:
            maxpossible = matrix[row][column]
            maxrow = row
            maxcolumn = column
...
direction(tracebackmatrix, maxcolumn,maxrow)
traceback.pop()
...
for item in traceback:
        if ctr1<len(seq1) and ctr2<len(seq2) and ctr2>=0 and ctr1>=0:
            if item == 'd':
